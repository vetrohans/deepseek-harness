...
describe('AppFrame', () => {
  ...
  it('renders the session pair with empty owner shares (sessionId is framework-standard)', () => {
    ...
    expect(slotCalls.find(c => c.key === 'preview')!.props).toEqual({ width: 0 })
  })
  ...
  it('drag base is the rendered (concession-clamped) width, not the preference', () => {
    frameWidth = 1030 // step-3 squeeze: details renders 350 while preference is 360
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openDetails() })
    expect(tracks(frame)).toEqual([280, 350, 0])
    const handles = frame.querySelectorAll('[class*="handle"]')
    drag(handles[1]!, 680, 690) // shrink by 10 from the rendered width
  ...
  it('preview column stays mounted at zero width', () => {
    const { frame, getByTestId } = mountFrame()
    expect(tracks(frame)).toEqual([280, 0, 0])
  ...
  it('preview opens, renders its slot with the live width, and drags to resize', () => {
    ...
    expect(slotCalls.filter(c => c.key === 'preview').at(-1)!.props).toEqual({ width: 360 })
    const handles = frame.querySelectorAll('[class*="handle"]')
    drag(handles[1]!, 1560, 1500) // shrink by 60 from the rendered width
