 ...
 * preview, then details, then auto-closing them (derived zero width —
 ...
 * SIDEBAR_COLLAPSED control rail while closed details/preview resolve to
 * zero width. The SIDEBAR_AUTO_COLLAPSE breakpoint is consumed by AppFrame,
...
/** Preview width before any user drag (open contract default). */
...
export function computeColumns(viewport: number, sidebar: number, details: number, preview: number): Columns {
  ...
  if (s + d0 + p0 + CENTER_MIN <= viewport) return { sidebar: s, center: viewport - s - d0 - p0, details: d0, preview: p0 }

  // Step 2: shrink preview toward its minimum (preview concedes before details).
  const p1 = p0 === 0 ? 0 : Math.max(PREVIEW_MIN, viewport - s - d0 - CENTER_MIN)
  if (s + d0 + p1 + CENTER_MIN <= viewport) return { sidebar: s, center: CENTER_MIN, details: d0, preview: p1 }

  // Step 3: shrink details toward its minimum.
  const d1 = d0 === 0 ? 0 : Math.max(DETAILS_MIN, viewport - s - CENTER_MIN)
  if (s + d1 + CENTER_MIN <= viewport) return { sidebar: s, center: CENTER_MIN, details: d1, preview: 0 }

  // Step 4: auto-close details and preview (derived — preferences untouched);
  // center absorbs any remaining deficit (may drop below CENTER_MIN).
  return { sidebar: s, center: Math.max(0, viewport - s), details: 0, preview: 0 }
}
