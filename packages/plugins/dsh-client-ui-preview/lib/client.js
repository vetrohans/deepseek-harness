window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// NOTE: .xRnBFa_ / .xmW8OG_ / .OVHKcG_ / .LPc7ja_ are CSS-module hashes of the
		...
		const navCss = [
			...
			"body:not([data-ds-dark-theme]) .rBF6Eq_navCell:hover,body:not([data-ds-dark-theme]) .rBF6Eq_navCell.rBF6Eq_active{background:#F3F3F3!important}",
			...
			"[data-composer-card] .dNVHna_primary{background:#000!important;width:30.6px;height:30.6px}",
			"[data-composer-card] .dNVHna_primary:hover:not(:disabled){background:#333!important}",
			"[data-composer-card] textarea::placeholder{font-size:14px;color:#A2A4A6}",
			"[data-composer-card] .dNVHna_primary svg:has(> path){transform:scale(.87);transform-origin:center}",
			...
			".xRnBFa_root{margin-left:calc(var(--dshqn-left) + (var(--dsw-composer-card-max-width) - var(--dsh-chat-content-width)) / 2)!important;margin-right:auto!important;margin-bottom:8px!important;text-align:center!important}",
			".xmW8OG_toBottomSlot{padding-right:max(0px,calc((100% - var(--dsh-chat-content-width))/2))!important}",
			"[data-queue-dock]{margin-left:calc(var(--dshqn-left) - 8px)!important}",
			".OVHKcG_root{margin-left:var(--dshqn-left)!important}",
			".LPc7ja_root{align-items:flex-start!important;padding-left:calc(var(--dshqn-left) + (var(--dsw-composer-card-max-width) - var(--dsh-chat-content-width)) / 2)!important}",
			...
			".Ysp4VW_projectRow .Ysp4VW_chevron{display:none!important}",
			".Ysp4VW_projectRow:hover .Ysp4VW_folder{display:inline-flex!important}",
			".Ysp4VW_projectRow[aria-expanded='true'] .Ysp4VW_folder{color:var(--dsw-alias-state-business-primary)!important}",
			// approval (等待审批) card: neutralize warn colors, align with composer
			".LPc7ja_card{border-color:var(--dsw-alias-border-l2)!important}",
			".LPc7ja_strip{background:var(--dsw-alias-bg-module-platform)!important;color:var(--dsw-alias-label-secondary)!important}",
			".LPc7ja_dot{background:var(--dsw-alias-label-secondary)!important}",
			".LPc7ja_root{align-items:flex-start!important;padding-left:calc(var(--dshqn-left) + (var(--dsw-composer-card-max-width) - var(--dsh-chat-content-width)) / 2)!important}",
			// "Deep diving…" running status: gray instead of blue
			".xmW8OG_turnStatus{background-image:linear-gradient(90deg, var(--dsw-alias-label-secondary) 0%, var(--dsw-alias-label-secondary) 40%, var(--dsw-static-neutral-200) 50%, var(--dsw-alias-label-secondary) 60%, var(--dsw-alias-label-secondary) 100%)!important}",
			".xmW8OG_turnStatusClock{color:#A2A4A6!important;-webkit-text-fill-color:#A2A4A6!important}",
			// sidebar rows: shorter hover rectangle
			".Ysp4VW_projectRow{height:32px!important}",
			".Ysp4VW_sessionRow{height:30px!important}",
			// sidebar workspace list: tighter vertical rhythm
			".sZU4WW_flatList>*+*,.sZU4WW_searchTree>[role=treeitem]+[role=treeitem],.sZU4WW_groupSection>*+*{margin-top:1px!important}",
			".sZU4WW_groupSection+.sZU4WW_groupSection{margin-top:2px!important}",
			".sZU4WW_sectionHeader{height:30px!important;margin-bottom:2px!important}",
			".sZU4WW_list{padding-bottom:8px!important}",
			// copy/action button under messages: hidden by default, shown on hover with the time
			"@media (hover:hover){[data-time-hover-root] ._8SXLmG_action{opacity:0;transition:opacity 80ms}[data-time-hover-root]:hover ._8SXLmG_action,[data-time-hover-root]:focus-within ._8SXLmG_action{opacity:1}}",
			// permission/access mode selector (Read-only/Workspace Write/Full access):
			// nudge it a bit to the left
			"[data-composer-card] .dNVHna_modes{margin-left:-16px!important}",
			// composer commands button: no gray circle at rest, keep hover fill
			"[data-composer-card] .dNVHna_add{background:transparent!important}",
			"[data-composer-card] .dNVHna_add:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-solid)!important}",
			// context-usage ring: used portion = conversation tab blue
			".H-3IJG_fill{stroke:var(--dsw-alias-state-business-primary)!important}",
			// Think/Bash tool-call message rows: tighter line spacing
			".AuyExq_summary,.ZK0AtW_summary{line-height:20px!important}",
			".AuyExq_thinkBody{line-height:20px!important;padding-top:2px!important;padding-bottom:2px!important}",
			".ZK0AtW_body{margin-top:2px!important;margin-bottom:2px!important}",
