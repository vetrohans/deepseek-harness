# Cordis 教程 · 第 2 章：生命周期与 effect

> 来源：<https://deepseek-harness.github.io/deepseek-harness/develop/cordis-tutorial/02-lifecycle-and-effects>
> 本文件是**本地参考副本**，存放在 `ui-overrides/docs/`（不会被上游合入覆盖）。
> 仓库里另有随上游同步的权威副本：`docs/cordis-tutorial/02-lifecycle-and-effects.zh.md`（整个系列 01–07 章都在 `docs/cordis-tutorial/`）。
>
> 适用场景：开发 dsh 插件时，凡是 Cordis 未管理的资源（定时器、连接、watcher、路由等），都要用 `ctx.effect()` 包裹并返回 disposer。
