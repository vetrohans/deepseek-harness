window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const navCss = [
			...
			"body:not([data-ds-dark-theme]) .hdil8G_navCell:hover,body:not([data-ds-dark-theme]) .hdil8G_navCell.hdil8G_active{background:#F3F3F3!important}",
		...
		"[data-phase]:has([data-conversation-scroll]){--dsh-chat-content-width:780px!important;--dsh-composer-card-max-width:780px!important}",
			...
			"[data-composer-card] ._7BGDkq_primary{background:#000!important;width:30.6px;height:30.6px}",
			"[data-composer-card] ._7BGDkq_primary:hover:not(:disabled){background:#333!important}",
			"[data-composer-card] textarea::placeholder{font-size:14px;color:#A2A4A6}",
			"[data-composer-card] ._7BGDkq_primary svg:has(> path){transform:scale(.87);transform-origin:center}",
			...
			".M_GpJq_toBottomSlot{padding-right:max(0px,calc((100% - var(--dsh-chat-content-width))/2))!important}",
			"[data-queue-dock]{margin-left:calc(var(--dshqn-left) - 8px)!important}",
			".eXUn6a_root{margin-left:var(--dshqn-left)!important}",
			".ToBpCG_root{align-items:flex-start!important;padding-left:calc(var(--dshqn-left) + (var(--dsw-composer-card-max-width) - var(--dsh-chat-content-width)) / 2)!important}",
			...
			".qJuGOG_projectRow .qJuGOG_chevron{display:none!important}",
			".qJuGOG_projectRow:hover .qJuGOG_folder{display:inline-flex!important}",
			".qJuGOG_projectRow[aria-expanded='true'] .qJuGOG_folder{color:var(--dsw-alias-state-business-primary)!important}",
			// approval (等待审批) card: neutralize warn colors, align with composer
			".ToBpCG_card{border-color:var(--dsw-alias-border-l2)!important}",
			".ToBpCG_strip{background:var(--dsw-alias-bg-module-platform)!important;color:var(--dsw-alias-label-secondary)!important}",
			".ToBpCG_dot{background:var(--dsw-alias-label-secondary)!important}",
			".ToBpCG_root{align-items:flex-start!important;padding-left:calc(var(--dshqn-left) + (var(--dsw-composer-card-max-width) - var(--dsh-chat-content-width)) / 2)!important}",
			// "Deep diving…" running status: gray instead of blue
			".M_GpJq_turnStatus{background-image:linear-gradient(90deg, var(--dsw-alias-label-secondary) 0%, var(--dsw-alias-label-secondary) 40%, var(--dsw-static-neutral-200) 50%, var(--dsw-alias-label-secondary) 60%, var(--dsw-alias-label-secondary) 100%)!important}",
			".M_GpJq_turnStatusClock{color:#A2A4A6!important;-webkit-text-fill-color:#A2A4A6!important}",
			// sidebar rows: shorter hover rectangle
			".qJuGOG_projectRow{height:32px!important}",
			".qJuGOG_sessionRow{height:30px!important}",
			// sidebar workspace list: tighter vertical rhythm
			".wLESWa_flatList>*+*,.wLESWa_searchTree>[role=treeitem]+[role=treeitem],.wLESWa_groupSection>*+*{margin-top:1px!important}",
			".wLESWa_groupSection+.wLESWa_groupSection{margin-top:2px!important}",
			".wLESWa_sectionHeader{height:30px!important;margin-bottom:2px!important}",
			".wLESWa_list{padding-bottom:8px!important}",
			// copy/action button under messages: hidden by default, shown on hover with the time
			"@media (hover:hover){[data-time-hover-root] .r1bZ5G_action{opacity:0;transition:opacity 80ms}[data-time-hover-root]:hover .r1bZ5G_action,[data-time-hover-root]:focus-within .r1bZ5G_action{opacity:1}}",
			// permission/access mode selector (Read-only/Workspace Write/Full access):
			// nudge it a bit to the left
			"[data-composer-card] ._7BGDkq_modes{margin-left:-16px!important}",
			// composer commands button: no gray circle at rest, keep hover fill
			"[data-composer-card] ._7BGDkq_add{background:transparent!important}",
			"[data-composer-card] ._7BGDkq_add:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-solid)!important}",
			// context-usage ring: used portion = conversation tab blue
			".s5M1Oa_fill{stroke:var(--dsw-alias-state-business-primary)!important}",
			// Think/Bash tool-call message rows: tighter line spacing
			".SI4i5a_summary,.vP6fIq_summary{line-height:20px!important}",
			".SI4i5a_thinkBody{line-height:20px!important;padding-top:2px!important;padding-bottom:2px!important}",
			".vP6fIq_bodyWrap{margin-top:2px!important;margin-bottom:2px!important}",
