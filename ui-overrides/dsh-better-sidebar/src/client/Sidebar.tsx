...
import { IconPanelBottomCloseOutline16, IconPanelBottomOutline16, IconPanelRightCloseOutline16, IconPanelRightOutline16 } from './icons.tsx'
        ...
        */}
        {!narrow && (
          <Tooltip label={state.bottomOpen ? t('collapseBottomPanel') : t('expandBottomPanel')} side="bottom" delayMs={500}>
            ...
            >
              {state.bottomOpen ? <IconPanelBottomCloseOutline16 /> : <IconPanelBottomOutline16 />}
            </button>
          </Tooltip>
        )}
        <Tooltip label={state.panelOpen ? t('collapse') : t('expand')} side="bottom" delayMs={500}>
          ...
          >
            {state.panelOpen ? <IconPanelRightCloseOutline16 /> : <IconPanelRightOutline16 />}
          </button>
        </Tooltip>
      </div>
      {/*
      ...
      */}
      ...
      <div
        ...
        style={{
          ...
          // While OPEN, force the visible state inline so a live toggle can
          // never be stranded by the collapsed-class CSS (visibility:hidden /
          // translateY(102%) not settling after the slide — observed as a
          // blank panel that only reappears after a full page refresh).
          // The class change still animates the slide (transform/visibility
          // are transitioned by .bottomPanel), this just pins the final state.
          ...(state.bottomOpen
            ? { visibility: 'visible' as const, transform: 'none' as const }
            : {}),
        }}
